...
def check_interest(reports: list[dict], text: str) -> list[tuple[str, str]]:
    ...
    for rep in reports:
        ...
        # 월 환산 (`prereg-30`) — 예금에는 숫자와 가정 문장이 같이, 적금에는 **없어야** 한다
        m = it.get("월환산")
        is_deposit = it["금액_뜻"] == "예치 금액"          # 리포트는 상품군 칸이 없다 — 이자 dict 의 뜻 라벨로 가른다
        if not is_deposit and m:
            bad.append((rep["상품"], "적금에 월 환산이 있다 — 매달 넣는 돈이 달라 성립하지 않는다"))
        if m:
            for label in ("세전", "세후"):
                for v in set(m[label]):
                    if C.won(v) not in text:
                        bad.append((rep["상품"], f"월 환산 {label} {C.won(v)} 가 화면에 없다"))
            if it["월환산_문장"] not in text:
                bad.append((rep["상품"], "월 환산의 가정 문장이 화면에 없다"))
        elif is_deposit:
            bad.append((rep["상품"], "예금인데 월 환산이 없다"))
...
def check_web(scored: list[dict], plan: dict, state: dict, total: int,
    ...
    **렌더러가 안 그리면 위반**이고, 그건 모델 겹이 못 잡는다.
    ...
    for s in vm["products"]:
        ...
        # A16 — 합쳐진 행의 숫자가 화면에 남아 있어야 한다 (`prereg-18` P8)
