...
# `기타` — 추출기가 17종 어디에도 못 넣은 조건. 남은 넷이 전부 **뜻이 공시에 없는 낱말**
# 이었다("소중한 날" · "올해의 띠" · "금리변동주기"). 뜻을 모르는 문구를 물으면 사용자도
# "모름" 밖에 못 답한다(3런 실측). 그래서 **판정 불가**로 다루고 묻지 않는다 — 사람 결정
# A(`prereg-19` · 이슈 #56). 라벨과 추출 스키마는 그대로다(세션 로그의 키 · `0031`).
# 사유 문장은 `추첨` 이 아니라 `뜻없음` 이다 — 추첨 문장을 붙이면 거짓이 된다
ETC_TYPE = "기타"
UNDECIDABLE = {"판정불가_불특정", UNCLEAR_TYPE, ETC_TYPE}   # 물어도 판정할 수 없다
...
def caveats_for(items_unknown: list[dict], items_met: list[dict], state: dict) -> list[str]:
    ...
    for it in items_unknown:
        ...
        if kind == UNCLEAR_TYPE:
            ...
            out.append("뜻없음")         # 뜻이 공시에 없는 낱말 — 묻지 않는다 (`prereg-19`)
...
#
# **링크가 생기면서 한 문장을 더했다** (F3 · `0037` D4). 기관 홈페이지·대표전화가 화면에
# 있으면 이 화면을 신청 창구로 읽을 수 있다 — 링크는 정보이고 가입은 기관에서 한다
...
CAVEAT = {
    ...
    # 다섯째 — 되물어도 못 없앤다 (`prereg-19` · 이슈 #56). `조건불명` 과 닮았지만 다르다 —
    # 조건불명은 조건 자체가 없고, 이것은 **조건 낱말은 있는데 뜻이 공시에 없다**
    # ("소중한 날"). 뜻을 지어내 묻지 않고(`0019`), 갈 곳을 알려주며 끝난다
    # 화면 낱말은 사람이 정했다(2026-09-04) — "뜻이 없다" 는 공시를 탓하는 말로 읽히고,
    # 사용자에게 필요한 것은 **할 일**이다. 코드 이름 `뜻없음` 은 화면에 안 나간다(A14)
    "뜻없음":    "어떤 경우에 받는지 공시에 설명이 없는 우대조건이 있어 그 부분은 확정하지 "
                 "않습니다. 해당되는지는 은행에 확인해 보세요",
}
...
def main() -> None:
    ...
    hard = [c for c in ("조건불명", "중복우대불명", "추첨", "단계불명", "모름", "뜻없음")
            if codes[c]]
    if hard:
        print()
