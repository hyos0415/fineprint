...
> 조건 구조를 명시적으로 표현하면, `Required − Claimed` 집합 차분이라는
> **결정론적 검사**로 이 실패를 잡을 수 있다.

## 이 프로젝트가 하지 않는 것

**"Graph를 써보는 프로젝트"가 아니다.** 위 가설이 참이라면 그래프 순회조차 필요 없을
수 있고, 그 가능성을 먼저 검증한다(`docs/handoff/v2.md`의 반증 게이트). 구조가 정말
필요하다고 확인된 뒤에만 그래프로 간다.
