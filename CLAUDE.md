...
**한국어를 포함한 모든 non-ASCII 문자는 코드 변환 없이 글자 그대로 출력할 것.
질문/선택지 UI의 라벨과 설명에도 동일하게 적용.**

구체적으로 세 가지다.

- **한자·일본어 약자를 한국어 자리에 섞지 않는다.** `両권역` 이 아니라 `두 권역`,
  `數値` 가 아니라 `수치`다. 2026-08-31 세션에서 실제로 `両권역` 이 나갔다
- **이스케이프·유니코드 코드포인트로 바꿔 쓰지 않는다.** `\uc138\ud6c4` 가 아니라
  `세후`다. JSON 을 쓸 때도 `ensure_ascii=False` 다 (이 저장소 스크립트가 이미 그렇게
  하고 있다)
- **깨져 보여도 문자를 바꾸지 않는다.** Windows 콘솔이 `cp949` 로 떨어져 출력이 깨지는
  것은 **터미널 문제**이고 파일 내용은 UTF-8 로 맞다. 깨짐을 피하려고 글자를 바꾸면
  파일이 틀리게 된다 — 파일이 맞았는지는 `git diff` 나 Read 로 확인한다
...
- **이슈는 에픽 아래에 붙인다** — 에픽 5개(#16~#20)가 부품 축이다(수집·운영 / 추출·검증 /
  사용자 입력 / 계산·출력·UI / 검색). **새 이슈는 반드시 에픽 하나에 붙이고**
  (`gh issue edit <에픽> --add-sub-issue <번호>`), 붙일 데가 없으면 에픽부터 만든다.
  **착수 가능한 것만 이슈로 판다** — 선행 조건 대기 항목은 `docs/spec/roadmap.md` 에만
  두고, 조건이 풀리는 순간 이슈를 만든다. 실험 결과에 따라 대기 항목이 바뀌므로
  미리 파두면 이슈 20개를 다시 쓰게 된다
- **전체 과업 지도는 `docs/spec/roadmap.md` 다.** 상태(✅▶⏸🙋)와 선행 조건은 그 문서에서만
  고친다. 에픽 이슈 본문에 내용을 옮겨 적지 않는다 — 원본이 두 군데면 서로 다른 말을 한다
- **브랜치**: `<이슈번호>-slug`
