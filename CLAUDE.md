...
**`START-HERE.md` → `docs/spec/problem.md` → `docs/spec/prereg-02-pilot.md` 순서로
읽는다.** 현재 상태와 다음에 할 일은 START-HERE, 문제 정의는 problem.md, 지금 진행 중인
파일럿 설계는 prereg-02에 있다.
...
한 줄 요약: AI가 예금·적금 금리를 설명할 때 **사용자가 실제로 받을 금리를 과대하게
말하는 것**을 잡아낸다 — 단, **구조나 그래프 없이 되는지부터 먼저 확인한다**
(파일럿 → v2 §3 반증 게이트).

## 절대 하지 말 것

1. **파일럿과 게이트(v2 §3)를 통과하기 전에 스키마·그래프 코드를 쓰지 않는다.**
