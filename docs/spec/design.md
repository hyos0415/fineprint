...
```
0단계  검색 축을 받는다     가입 기간 · 월 납입액(적금) / 예치금(예금) · 권역
...
쓴다(`0018`은 원래 질문 부풀리기 방지 목적이었다). 전부 답하면 **은행권 22개 ·
저축은행 27개**이고, 행원 면담 한 번 분량이다.
...
```
중단 지점       22~27개 중간에 그만두면 P2(질문 수 제한 없음)가 틀렸다
0번째 중단      분모 22개가 시작 자체를 막은 것이다 → B안(2단계 분할)으로 돌아간다
"모르겠다" 비율  절반을 넘으면 질문이 답할 수 없는 형태다 → P1(공시 문구 원문)을 다시 본다
```

**답을 읽을 때 정한 것 둘**(`decisions/0025`) — 수치 질문의 "아니오"는 **그 조건 자체를
안 한다**는 뜻으로 유형 쪽을 `False`로 되돌린다(0으로 넣으면 `"1천만원 이하"` 같은 최대
방향 임계를 오히려 충족시킨다). 수치 질문에 "모르겠다"를 답하면 **다시 묻지 않는다** —
루프를 만들기 전에는 이 경로가 무한히 되물었다.
...
recommend_*.json 한 상품이 담는 것
  ...
  net_lo · net_hi         세후 범위        ← 정렬은 net_lo 로 한다
...
- **2026-08-28에 그 검사를 만들었다** — `src/analysis/check_screen_contract.py`.
  질문 0개부터 끝까지 **모든 중간 상태**에서 A1~A5를 본다(사용자가 12번째 질문에서
...
| # | 실험 | 선행 조건 | 상태 |
|---|---|---|---|
| E1 | 폴백 구조 사전등록 후 재측정 | 없음 | **완료** — 은행권 83.9%(예측 83.8%). `prereg-04` · `0012` |
...
| E10 | 과다 추출의 원인 분류 | 없음 | **완료** — 96.7%가 폭 0 행(공시 문제), 진짜 과다는 23행 |
| E11 | `threshold` 를 스키마에 넣으면 과다 추출이 줄어드나 | 재추출 $1 | **보류** — `0013`. 대상이 넘침 12~13행뿐이고 그중 일부만 threshold 원인이다. 사용자별 계산에는 여전히 필요 |
| E12 | 기관 group-by 로 충분한가 | 없음 | **완료** — 기관 2.17배 · 상품군 1.25배. 남는 구조 0.3%. `0014` |
...
구현됨
  ...
  화면 계약    src/analysis/check_screen_contract.py  중간 상태 전수 assert (A1~A5)

미구현
  R2 · R3 · R4 · 편의 속성 결합 · 리포트 · UI(저장소에 .html·.js 0개)
  RAG · 리랭커 · 라우터 · Airflow
```
