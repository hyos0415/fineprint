...
- **상태**: **스코프 다섯 칸까지 구현** (2026-09-09 · `0060` · `prereg-29` · `r2_parse.prefill()` · `POST /prefill`). 0단계 폼의
  "내 상황" 상자를 로컬 모델(llama.cpp · Qwen3.5-4B)이 읽어 권역·은행·예금/적금·기간·금액을 **폼에 미리 채우고**, 사용자가 고쳐서 낸다.
  **조건 답은 채우지 않는다** — 세 표본 100문장에서 어려운 표본 오답 30% 로 기각(`0058`·`0060`). 거래 은행 목록도 채우지 않는다
- **없으면**: 폼으로 받으면 된다. 시스템은 돈다. 편의성만 떨어진다 — 상자를 비우거나 모델 서버가 없으면 지금과 같다
- **측정**: 스코프 다섯 칸 정확도 TEST2 89%(`prereg-29` P0). 남은 측정은 **사람 세션에서 고친 칸 비율**(관문 20% · `0060` 반증 조건 1)
...
### 비교 리포트 — 숫자 칸은 코드가 채운다 (F1 · `0036`)

```
계약  입력: evaluate() 가 낸 상품 dict (`why` 포함)
      출력: 리포트 객체 + 텍스트 렌더
      금리를 **다시 계산하지 않는다** — 이미 계산된 값을 옮겨 적는다
```

리포트가 반드시 담는 것 여섯.

1. **계단** — 기본금리 → 확실히 받는 우대 → 불확실한 우대 → 세전 → 세금 → 세후
2. **조건별 %p 와 공시 문구 원문** — 문구를 **자르지 않는다**(`0027`)
3. **합이 안 맞으면 그 차이를 적는다** — 조건별 단순 합 ≠ 계산에 들어간 값이 되는
   자리가 둘이다(배타 그룹 `0022` · 상품 상한 `cap`)
4. **최대값이 어떤 가정 위에 서 있는지** — 낙관 쪽은 배타 그룹을 **믿지 않고 다
   더한다**. 실측에서 한 상품이 같은 그룹 세 조건으로 `+12.00%p` 였다
5. **근거 없는 %p** — `⚠ 광고 최고금리 중 X%p 는 근거가 공시에 없다`
6. **왜 이 순서인가** — 정렬 기준과 선호 조정(%p·사유). 조정은 순서만 바꾼다(A11)

- **없으면**: 목록 한 줄이 전부다. 2026-08-31 재고 조사에서 상위 10개 상품의 추출
  항목 27개 중 **공시 문구가 화면에 있는 것 0개** · 화면 1,590자에 `%p` 0회였다.
  `problem.md` §3 이 약속한 칸 14개 중 4개가 없었다
- **측정**: 화면 계약 assert 를 리포트에도 건다 — A1(범위)·A3(사유 문장)·A12·A13.
  위반은 `detail` 에 `[리포트]` 가 붙는다
...
**예외가 하나 생겼다 — 맨 처음 묻는 목록 질문이다** (`0045` · F6 · 2026-09-02).

조건 유형 6종은 **그 기관과의 관계**다(첫거래 · 주거래 · 급여이체 · 카드실적 ·
자동이체 · 타상품 보유). 그런데 답이 기관과 무관한 전역 값이라 **답 하나가 기관 10곳
상품 22개를 한꺼번에 올렸고**, 한 기관 안에서 첫거래와 주거래를 동시에 줬다
(애큐온 처음만난적금 `+3.50` + `+2.00` = **+5.50%p 를 다 줬다**).

그래서 답을 `자동이체@국민은행` 처럼 **기관이 붙은 키**로 받는다. 기관마다 예/아니오로
물으면 은행권에서 (유형, 기관) 쌍이 **56개**가 되므로, 목록 질문 하나로 묻고 나머지를
유도한다.

```
① 한 번 묻는다   "거래해 본 기관을 골라 주세요" (여러 곳 가능 · 화면은 체크박스)
② 안 고른 기관    첫거래 = 예 · 그 기관의 실적 조건 전부 아니오
③ 고른 기관      6종을 되묻는다. 단 한 기관 안에서 첫거래와 주거래는 배타다
④ "모름"        유도하지 않고 기관마다 따로 묻는다 — 우리가 대신 정하지 않는다
```

**유도는 추측이 아니다.** 우리가 모르는 것을 채우는 게 아니라, 사용자가 이미 말한 것의
필연적 귀결을 두 번 묻지 않는 것이다 — *거래가 없으면 그 기관에 실적이 있을 수 없고,
신규이면서 동시에 주거래일 수 없다.* 그래서 유도는 그 둘로 제한했다.

**유도된 답은 상태에 저장하지 않는다.** 읽을 때 `calculate.answer_of()` 가 계산한다 —
그래야 화면의 *"답한 질문 N개"* 가 **사용자가 실제로 답한 수**로 남는다(A6).

#### 화면 낱말 — 우리가 지은 이름은 화면에 안 내보낸다 (F5 · `0046`)
...
없앤다     내부 이름·코드처럼 만든 사람만 아는 것
          ...
          `미응답` → `안 답한 조건 있음` · `스코프` → `찾는 범위`
설명한다   `%p`·`공시`·`세후`·`우대조건` — 정확해서 필요하다. 첫 등장에서 풀어 준다
```

표는 `calculate.py` 한 곳(`TYPE_LABEL`·`TIER_LABEL`·`CAVEAT_LABEL`)이고 렌더러 셋이
전부 그것을 읽는다. **키는 안 바꾼다** — 추출 스키마의 enum 이고 세션 로그·옛 측정의
키다(`0031` 이 겪은 자리). 그 대가는 **A14** 가 갚는다.

**`0019` 와 다르다.** `0019` 는 *뜻을 모르는 남의 낱말*("탑스")에 뜻을 지어내는 것을
막았고, 여기는 **우리가 지은 낱말**이라 뜻을 정확히 안다 — 지어내는 것이 아니라 번역이다.
...
**화면 필수 항목 열.** (1~4 가 원래 넷이고 5~10 은 뒤에 붙었다)
...
8. **금리 숫자 옆에 `세전`·`세후` 를 반드시 쓴다** (2026-08-31 · `0034`).
   ...
   *"수익률(이자율) 표기시 '세전'인지 '세후'인지를 누락하여 표시·광고하는 것은
   ...
   표시광고법을 함께 걸어 둔다).
   > **정정 (2026-08-31 · `0035`).** 처음 이 항목을 *"CLI 는 이미 지키고 있다"* 로
   > 적었는데 **`calculate.py` 목록 헤더만 보고 쓴 말이었다.** 사용자가 실제로 보는
   > `ask_loop` 최종 화면에는 `세후` 라는 말이 **한 번도 없었다**. 화면 맨 위에
   > *"금리는 모두 세후입니다 — 이자소득세 등 15.4% 를 뗀 값입니다"* 를 넣고
   > **assert A12** 로 박았다(세율은 config 에서 읽는다 — 문자열에 안 박는다).
   > 헤더를 일부러 빼고 돌려 **불통과 156건**이 나오는 것까지 확인했다
9. **화면이 자기가 무엇인지 밝힌다** (`0035` · assert A13 · 이슈 #31).
   화면 맨 위에 *"이 화면은 금융감독원 공시를 계산한 결과입니다 — 상품을
   판매·중개하지 않고, 특정 상품을 권하지 않습니다"* 가 항상 있다.
   **"광고가 아니다" 라고 쓰지 않는다** — 그건 법적 판정이고 우리가 내릴 것이
   아니다(#31 은 열려 있다). 대신 **우리가 하는 일과 안 하는 일**을 적는다.
   문구는 `calculate.NOTICE` 한 곳에 있고 두 화면과 검사기가 그것을 읽는다 —
   사본을 만들면 한쪽만 고쳐도 통과한다
10. **선호가 걸리면 성과 줄을 `최고` 가 아니라 `1위` 라고 쓴다** (`0030`).
...
- **2026-08-28에 그 검사를 만들었다** — `src/analysis/check_screen_contract.py`.
  질문 0개부터 끝까지 **모든 중간 상태**에서 A1~A19를 본다(사용자가 12번째 질문에서
...
구현됨
  ...
  화면 계약    src/analysis/check_screen_contract.py  중간 상태 전수 assert (A1~A19)
...
미구현
  R3 · R4 (R2 는 스코프 다섯 칸까지 됐다 · 조건 답은 기각 — `0060`)
